  ...
  return (
    <Paper shadow="sm" withBorder py="md" px={5} h="max-content" mt="md" w={uiStore.inputWidth}>
      ...
      <Stack spacing={5} mah={550} px="sm" style={{overflowY: "auto", overflowX: "visible", overscrollBehavior: "none"}}>
        {
          content.map(pocketMediaItem =>
            <MediaItemCard
              key={`media-item-${pocketMediaItem.id}`}
              mediaItem={pocketMediaItem.use_media_settings ? pocketMediaItem.mediaItem : pocketMediaItem}
              aspectRatio="Canonical"
              size="sm"
              withLink
              link={UrlJoin("/pocket", pocketId, "media", pocketMediaItem.id)}
              showPermissions
            />
          )
        }
