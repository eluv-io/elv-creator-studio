...
const NFTDetails = observer(({nft, initialListingStatus, item, giftItem, hideSecondaryStats}) => {
  ...
  if(nftInfo?.hasBundledProperty && nftInfo?.isOwned) {
    return <Navigate to={UrlJoin("/m", match.params.contractId, match.params.tokenId, "p", nftInfo.bundledPropertyId, SearchParams()?.page === "details" ? "details" : "")} />;
  }

  // Marketplace item claimed
  if(claimed) {
    return <Navigate to={UrlJoin("/marketplace", match.params.marketplaceId, "store", item.sku, "claim", "status")} />;
  }

  // Pack opened
  if(opened) {
    return match.params.marketplaceId ?
      <Navigate to={UrlJoin("/marketplace", match.params.marketplaceId, "users", "me", "items", match.params.contractId, match.params.tokenId, "open")} /> :
      <Navigate to={UrlJoin("/wallet", "users", "me", "items", match.params.contractId, match.params.tokenId, "open")} />;
  }

  // NFT Burned
  if(burned) {
    return match.params.marketplaceId ?
      <Navigate to={UrlJoin("/marketplace", match.params.marketplaceId, "users", "me", "items")}/> :
      <Navigate to={Path.dirname(Path.dirname(match.url))}/>;
  }
  ...
  if(["owned", "owned-media"].includes(SearchParams()["redirect"])) {
    if(ownedItemLoading) {
      ...
      return match.params.marketplaceId ?
        <Navigate to={UrlJoin("/marketplace", match.params.marketplaceId, "users", "me", "items", ownedItem.contractId, ownedItem.tokenId, mediaPage ? "media" : "")}/> :
        <Navigate to={UrlJoin("/wallet", "users", "me", "items", ownedItem.contractId, ownedItem.tokenId, mediaPage ? "media" : "")}/>;
    }
  }
...
export const MintedNFTRedirect = observer(() => {
  ...
  return (
    <AsyncComponent
      ...
      render={() =>
        tokenId ? <Navigate to={UrlJoin(match.url, tokenId)} /> :
          notFound ? <Navigate to={Path.dirname(match.url)} /> :
            null
      }
    />
  );
