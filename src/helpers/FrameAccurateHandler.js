...
class FrameAccurateHandler {
  constructor({frameRate, frameRateRat}) {
    this.SetFrameRate({rate: frameRate, rateRat: frameRateRat});

    // Only set drop frame if appropriate based on frame rate
    const frameRateKey = FrameAccurateHandler.FractionToRateKey(`${this.frameRateNumerator}/${this.frameRateDenominator}`);
    this.dropFrame = ["NTSC", "NTSCFilm", "NTSCHD"].includes(frameRateKey);
  }
  ...
  SetFrameRate({rate, rateRat}) {
    ...
    if(rateRat) {
      ...
      const rateKey = FrameAccurateHandler.FractionToRateKey(this.frameRate);
...
export default FrameAccurateHandler;
